...
export const unlinkInvalidations = async (queryContext: QueryContext, sliceDbIds: number[]) => {
  const { dataSource, timeRange } = queryContext;
  if (sliceDbIds.length === 0) {
    return Promise.reject('Nothing to unlink');
  }
  const table = `planned_slice`;
  const sliceDbIdsString = sliceDbIds.join(',');
  const rawSql = `update ${table} set invalidation_id = null where ${table}.id IN(${sliceDbIdsString})`;
