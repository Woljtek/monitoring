  ...
  * [Trace ingestor](#trace-ingestor)
    * [Configuration file](#configuration-file-1)
      * [Mapping](#mappings)
        ...
        * [Duplicate query property](#duplicate-query-property)
...
| duplicate_query        |           | Set SQL start point to find processing that must be set as duplicated. The process start before ingesting entity associated to the current trace.                                                                                                                                                                                                                                                                                                                                   | An SQL query (see [SQL syntax](https://www.postgresql.org/docs/14/index.html)) |
...
##### Duplicate query property

The duplicate query property is used to set already ingested processing as duplicated. A processing is considered duplicated when it match query conditions. From this processing, all chained processing are also set duplicated. Processing are chained by output product (cf. output_list entity) which is the input product (cf. input_list_internal entity) of the next processing.
...
For example with the above table, we require to start from input which are chunk or dsib so processing from l0 to l1 will be set as duplicated.

You can customize above query by adding further conditions to match a specific processing such as the rs chain name (cf field processing.rs_chain_name) and other field attached to processing entity:
